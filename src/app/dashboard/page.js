           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.5rem' }}>
             {products.length === 0 ? (
               ...
               products.map((product, index) => (
                 <div key={product.id} className={`row-hover animate-fade-in-up delay-${Math.min((index + 4) * 100, 500)}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem', background: 'var(--background)', borderRadius: '12px', border: '1px solid var(--border)' }}>
                    <div>
